...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService
{
    ...
    {
        //设计并且组装提示词，需要从redis中获取数据，并且拼接提示词
       String prompt = getPrompt(healthAssessment.getIdCard());
        //调用大模型，分析数据得到结果
        String tongYiResult = aiModelInvoker.TongYiInvoker(prompt);
        //解析结果，将数据保存在数据库中，并且返回主键id
        HealthReportVo healthReportVo = JSON.parseObject(tongYiResult, HealthReportVo.class);
    ...
    private String getPrompt(String idCard) {
        ...
        String prompt = "请以一个专业医生的视角来分析这份体检报告，报告中包含了一些异常数据，我需要您对这些数据进行解读，并给出相应的健康建议。\n" +
                ...
                "\n" +
                "输出要求：\n" +
                "最后，将以上结果以json格式输出，不要包含其他的文字说明，也不要出现Markdown语法相关的文字，所有的返回结果都是json格式，详细格式如下：\n" +  // 修改这里，明确提到"json格式"
                "\n" +
                "{\n" +
